...
"""Returns trellis with alpha probabilities given input"""
    ...
    for x in xrange(1, len_inputs):
        ...
        for ti in word_tag[current_word]:
            ...
            for y in xrange(len(forward_trellis[x - 1])):
                ...
                p = prob_tagtag(ti, ti_1, 1) + \
                    prob_wordtag(inputs[x], ti, 1) + alpha_1
                # p = add1_prob_tagtag(ti,ti_1,1+sing_tt[ti_1]) +\
                # add1_prob_wordtag(inputs[x],ti,1+sing_tw[ti])+alpha_1
...
"""Returns trellis with beta probablites given input"""
    ...
    for x in xrange(len_inputs - 2, -1, -1):
        ...
        for ti in word_tag[current_word]:
            ...
            for y in xrange(len(backward_trellis[len_inputs - 2 - x])):
                ...
                p = prob_tagtag(ti_1, ti, 1) + \
                    prob_wordtag(inputs[x + 1], ti_1, 1) + beta_1
                # p = add1_prob_tagtag(ti_1,ti,1+sing_tt[ti]) +\
                #  add1_prob_wordtag(inputs[x+1],ti_1,1+sing_tw[ti_1])+beta_1
...
"""Compute joint probability using alpha and beta values"""
    ...
    for i in xrange(len(alpha_values)):
        ...
        for j in xrange(len(alpha_values[i])):
            if alpha_values[i][j][0] == beta_values[len_beta_values - 1 - i][j][0]:
                ...
                temp_alpha_beta.append((alpha_values[i][j][0], alpha_values[i][j][
                                       1] + beta_values[len_beta_values - 1 - i][j][1]))
    ...
    for x in xrange(1, len(alpha_beta_values)):
        ...
        for y in xrange(len(alpha_beta_values[x])):
            ...
            for z in xrange(len(alpha_beta_values[x - 1])):
                ...
                p_tag = math.exp(alpha_values[x - 1][z][1] + beta_values[x][y][1] + prob_wordtag(
                    curr_word, curr_tag, 1) + prob_tagtag(curr_tag, tag_prev, 1) - logS)
...
def doFwdBck(a, iter):
    ...
    alpha_values, S = compute_alpha(a)
    ...
    dt_new, d_tt_new, d_tw_new = get_countsfrom_AlphaBeta(
        a, alpha_beta_values, alpha_values, reverse_beta_values, S)
    return dt_new, d_tt_new, d_tw_new
    ...
    for x in xrange(1, len_a):  # ignore the first test_word which is ###
        ...
        for ti in word_tag[temp]:
            ...
            for y in xrange(len(b[x - 1])):
                ti_1 = b[x - 1][y][0]
                mu_1 = b[x - 1][y][2]
                temp_mu = prob_tagtag(ti, ti_1, 1) + \
                    prob_wordtag(a[x], ti, 1) + mu_1
...
if __name__ == "__main__":
    ...
    # now vocabulary includes total distinct words in raw and train
    V = len(dw.keys()) + 1
